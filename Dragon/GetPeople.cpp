...
//	m_sex_id		= dlg._sex_id;
	...
	if( m_fullname.IsEmpty() )
		m_command.Format( L"SELECT rowid,* FROM people WHERE sex_id='%d'",  m_sex_id );
	else if( m_first_name.IsEmpty() )
		m_command.Format( L"SELECT rowid,* FROM people WHERE last_name LIKE '%s%c' AND sex_id='%d'", m_last_name,'%', m_sex_id );
	else
	...
	for( UINT i = 0; i < theApp.m_recordset2->RecordsCount(); ++i, theApp.m_recordset2->MoveNext() )
	{
		...
		nItem = m_ListCtrl.InsertItem( nItem, theApp.m_recordset2->GetFieldString( PEOPLE_ROWID ) );
		m_ListCtrl.SetItemText( nItem, L_GENERATION, theApp.m_recordset2->GetFieldString( PEOPLE_GENERATION ) );
		m_ListCtrl.SetItemText( nItem, L_LAST_NAME, theApp.m_recordset2->GetFieldString( PEOPLE_LAST_NAME ) );
		m_ListCtrl.SetItemText( nItem, L_FIRST_NAME, theApp.m_recordset2->GetFieldString( PEOPLE_FIRST_NAME ) );
		m_ListCtrl.SetItemText( nItem, L_BIRTH_PLACE, theApp.m_recordset2->GetFieldString( PEOPLE_BIRTH_PLACE ) );
		m_ListCtrl.SetItemText( nItem, L_BIRTH_DATE, theApp.m_recordset2->GetFieldString( PEOPLE_BIRTH_DATE ) );
		m_ListCtrl.SetItemText( nItem, L_DEATH_PLACE, theApp.m_recordset2->GetFieldString( PEOPLE_DEATH_PLACE ) );
		m_ListCtrl.SetItemText( nItem, L_DEATH_DATE, theApp.m_recordset2->GetFieldString( PEOPLE_DEATH_DATE ) );
		m_ListCtrl.SetItemText( nItem, L_COMMENT, theApp.m_recordset2->GetFieldString( PEOPLE_COMMENT ) );
	}
