...
bool CPictures::showPdf( int nItem )
{
	...
	ShellExecute(NULL,L"open", theApp.m_pdfName, str, theApp.m_pdfFolder, SW_SHOW | SW_SHOWNORMAL);
...
void CPictures::OnClickedView()
{
	...
	{
		ShellExecute(NULL,L"open", theApp.m_pdfName, m_fileSpec, theApp.m_pdfFolder, SW_SHOW | SW_SHOWNORMAL);
	}
	else if( m_ext == L"doc" || m_ext == L"docx" || m_ext == L"xls" || m_ext == L"xlsx" ) 
