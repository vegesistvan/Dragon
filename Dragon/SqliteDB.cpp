...
bool CSqliteDB::Connect(LPCTSTR strDBPathName, LPCTSTR strDBFilePassword, int Flags)//adatbazis megnyitasa (csatlakozas)
{
...
#if !defined (_UNICODE) && !defined (UNICODE)
	dbName=UnicodeToUtf8(MultiToUnicode(strDBPathName));
#endif
	...
	else
	{
		CStringA strUtf8;
#if defined (_UNICODE) || defined (UNICODE)
		strUtf8=UnicodeToUtf8(strDBPathName);// az openV2 csak utf8-al mukodik
#else
		strUtf8=dbName;
#endif		
	...
	if (dbFilePass.Compare(_T(""))!=0)
	{
		if(m_encryption)
		{
			CStringA strPass;
#if !defined(_UNICODE) && !defined(UNICODE)
			strPass=UnicodeToUtf8(MultiToUnicode(strDBFilePassword));
#else
			strPass=UnicodeToUtf8(strDBFilePassword);
#endif
...
int CSqliteDB::ChangePass(LPCTSTR strNewPass)// adatbazis jelszavanak megvaltoztatasa
{
	if ((m_bConnected&1)==1)
	{
		if(m_encryption)
		{
			CStringA str;
#if !defined(_UNICODE) && !defined(UNICODE)
			str=UnicodeToUtf8(MultiToUnicode(strNewPass));
#else
			str=UnicodeToUtf8(strNewPass);
#endif
...
#if defined (_UNICODE) || defined (UNICODE)
	str=Utf8ToUnicode(libver);
#else
	str=UnicodeToMulti(Utf8ToUnicode(libver));
#endif
	return str;
}
...
int CSqliteDB::Execute(LPCTSTR strQuery)  // sql parancs kiadasa adatbazis modositas celjabol. lekerdezes nem lehetseges
{
	...
	CStringA str;
#if !defined(_UNICODE) && !defined(UNICODE)
	str=UnicodeToUtf8(MultiToUnicode(strQuery));
#else
	//str=UnicodeToUtf8(strQuery);
	str=UnicodeToUtf8(strQuery);
#endif
...
#if defined(_UNICODE) || defined(UNICODE)
	ErrMsg=Utf8ToUnicode(zErrMsg).GetBuffer(0);
#else
	ErrMsg=zErrMsg;
#endif
...
UINT CSqliteDBRecordSet::Query(LPCTSTR strQueryString, void *ptrDB, int firstElement, int maxElement)// a rekordszet feltoltese sql paranccsal
{
...
#if !defined(_UNICODE) && !defined(UNICODE)
	strBuffer=UnicodeToUtf8(MultiToUnicode(strQueryString));
#endif
	...
	{
#if !defined(_UNICODE) && !defined(UNICODE)
		strBuffer=UnicodeToMulti(Utf8ToUnicode(SqliteDBColumnName (stmtByteCode, i))); // az oszlopnevet
#else
...
/*			int fType=SqliteDBColumnType (stmtByteCode, i);// minden mezo erteket a tipusa alapjan adunk vissza
			...
			//switch (nColumnsType.GetAt(i))
			{
				...
				case SQLITE_TEXT:*/
#if !defined(_UNICODE) && !defined(UNICODE)
					strBuffer = UnicodeToMulti(Utf8ToUnicode(SqliteDBColumnText (stmtByteCode, i)));
#else
