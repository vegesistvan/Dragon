...
void CGEDCOMInput::OnBnClickedOk()
{
	...
	if (str.IsEmpty())
	{
		AfxMessageBox(L"A bementi fájlt meg kell adni!");
		return;
	}
	CDialogEx::OnOK();
	gedcomInput();
...
bool CGEDCOMInput::gedcomInput()
{
	...
	if( AfxMessageBox( L"Az adatbázis feltöltése befezõdött.\nListázzam a vektorokat?", MB_ICONQUESTION|MB_YESNO ) == IDYES )
		printVectors();
