...
BOOL CSelectMother::OnInitDialog()
{
	...
	if( m_rowidF.IsEmpty() || !m_rowidF.Compare( L"0" ) )
	{
		...
		m_command.Format( L"SELECT spouse1_id FROM marriages WHERE spouse2_id='%s'", m_rowidM );
		...
		else
		{
			for( UINT i = 0; i < theApp.m_recordset->RecordsCount(); ++i, theApp.m_recordset->MoveNext() )
			{
				...
				m_ListCtrl.SetItemText( nItem, L_ROWID, theApp.m_recordset2->GetFieldString( PEOPLE_ROWID ) );
				mother.Format( L"%s %s", theApp.m_recordset2->GetFieldString( PEOPLE_LAST_NAME ), theApp.m_recordset2->GetFieldString( PEOPLE_FIRST_NAME ) );
				m_ListCtrl.SetItemText( nItem, L_MOTHER, mother );
				birth = theApp.m_recordset2->GetFieldString( PEOPLE_BIRTH_DATE );
				m_ListCtrl.SetItemText( nItem, L_BIRTH, birth );
				death = theApp.m_recordset2->GetFieldString( PEOPLE_DEATH_DATE );
...
//		SetWindowPos(&wndTopMost, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	...
	{
		m_command.Format( L"SELECT spouse2_id FROM marriages WHERE spouse1_id='%s'", m_rowidF );
		if( !theApp.query( m_command ) ) return FALSE;
