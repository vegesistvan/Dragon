...
//bool sortByFather(const NEWFAMILIES &v1, const NEWFAMILIES &v2);
//bool sortByMother(const NEWFAMILIES &v1, const NEWFAMILIES &v2);
// CGedcomOut dialog
...
BOOL CGedcomOut::OnInitDialog()
{
	...
	if( m_filter.IsEmpty() ) m_filter = theApp.m_baseName;
	m_fileName.Format( L"%s.ged", m_filter );
	GetDlgItem( IDC_FILENAME )->SetWindowText( m_fileName );


	m_gedFolder.Format( L"%s\\ged", theApp.m_databasePath );
	GetDlgItem( IDC_PATH )->SetWindowText( m_gedFolder );
	return TRUE;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CGedcomOut::OnBnClickedOk()
{
	...
	m_gedFolder.Format( L"%s\\ged", theApp.m_databasePath );
	if( _waccess( m_gedFolder, 0 ) ) _wmkdir( m_gedFolder );

	m_relFolder.Format( L"%s_BLOB", m_fileName );

	m_blobFolder.Format( L"%s\\%s", m_gedFolder, m_relFolder );
	...
	// ged fájl készítése
	m_gedFileSpec.Format( L"%s\\%s", m_gedFolder, m_fileName );
	if( !openFileSpec( &fg, m_gedFileSpec, L"w+" ) ) return;
	...
	if( theApp.NOTEPAD )
	{
		theApp.notepad( m_gedFileSpec, L"-alwaysOnTop -nosession" );
		theApp.notepad( fileSpec, L"-alwaysOnTop" );
	}
	else
	{
		str.Format( L"%s\nelkészült!", m_gedFileSpec );
		AfxMessageBox( str );
	}
...
//	vIndi.fams_id = nincs;
	...
	for( UINT i = 0; i < p_ListCtrl->GetItemCount(); ++i )
	{
		if( theApp.m_inputMode == MANUAL ) 
		{
			...
			vIndi.comment		= p_ListCtrl->GetItemText( i, N_COMMENT );
			vIndi.rowidF		= _wtoi( p_ListCtrl->GetItemText( i, N_ROWID_FATHER ) );
			vIndi.rowidM		= _wtoi( p_ListCtrl->GetItemText( i, N_ROWID_MOTHER ) );
		...
		if( theApp.m_inputMode == GEDCOM ) 
		{
			...
			vIndi.rowidF		= _wtoi( p_ListCtrl->GetItemText( i, G_ROWID_FATHER ) );
			vIndi.rowidM		= _wtoi( p_ListCtrl->GetItemText( i, G_ROWID_MOTHER ) );
		...
		if( theApp.m_inputMode == GAHTML ) 
		{
			...
			vIndi.rowidF		= _wtoi( p_ListCtrl->GetItemText( i, H_ROWID_FATHER ) );
			vIndi.rowidM		= _wtoi( p_ListCtrl->GetItemText( i, H_ROWID_MOTHER ) );
...
// vCHIL vektor elkészítése
	...
	for( i = 0; i < vFAM.size(); ++i )
	{
		...
		// a házaspár minden gyerekét beteszi a vCHIL_be
...
void CGedcomOut::list()
{
	...
	fwprintf( fl, L"\n\nFAM rekordok:\n\n" );
	fwprintf( fl, L"%-8s %-8s %-8s %-30s %-30s\n", L"FAM_ID", L"HUSB_ID", L"WIFE_ID", L"férj", L"feleség" );
	for( UINT i = 0; i < vFAM.size(); ++i )
	{
		wife.Empty();
		husbIndex = vFAM.at(i).husb_id;
