...
{
	gedPathName = theApp.m_gedPathName;
	if (gedPathName.IsEmpty())
	{
		getGedcomPathName();
...
void CGedcomFile::listFileTab()
{
	...
	CString fileSpec = theApp.openTextFile(&theApp.fl, theApp.m_gedFileTitle, L"w+");
	fwprintf(theApp.fl, L"%s\n\n", gedPathName);


	str.Format(L"Beolvasása a %s fájlból...", theApp.m_gedFileName);
...
void CGedcomFile::listZero()
{
	...
	str.Format(L"Rekord fejlécek beolvasása a %s fájlból...", theApp.m_gedFileName );
	CProgressWnd wndP(NULL, str);
	wndP.GoModal();
	wndP.SetRange(0, (int)gedFile.GetLength());
	while (gedFile.ReadString(cLine))
...
void CGedcomFile::listHEAD()
{
	...
	str.Format(L"Fejléc rekordok beolvasása a %s fájlból...", theApp.m_gedFileName);
	CProgressWnd wndP(NULL, str);
	wndP.GoModal();
	wndP.SetRange(0, (int)gedFile.GetLength());
...
void CGedcomFile::filterIrregular()
{
	...
	newGedFile.Format(L"%s\\%s_regular.ged", theApp.m_dbFolderPath, theApp.m_gedFileTitle);

	if (!openFileSpec(&fl, newGedFile, L"w+")) return;

	str.Format(L"%s fájl szûrése folyik...", theApp.m_gedFileName);
	CProgressWnd wndP(NULL, str);
	wndP.GoModal();
	wndP.SetRange(0, (int)gedFile.GetLength());
	wndP.SetPos(0);
...
bool CGedcomFile::getGedcomPathName()
{
	...
	splitFilespec(gedPathName, &drive, &path, &theApp.m_gedFileTitle, &ext);

	theApp.m_gedFolderPath.Format(L"%s:%s", drive, path);
	theApp.m_gedFileName.Format(L"%s.%s", theApp.m_gedFileTitle, ext);
	return true;
}