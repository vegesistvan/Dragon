...
void CCheckMotherDeath::motherDeathChildBirth()
{
	...
	for( UINT i = 0; i < m_F->RecordsCount(); ++i, m_F->MoveNext() )
	{
		...
		// apa feleségei
		m_command.Format( L"SELECT rowid, * FROM marriages WHERE spouse1_id='%s' ORDER BY orderWife", father_id );
		if( !query2( m_command ) ) return;

		// feleségek ciklusa
		for( j = 0; j < m_recordset2->RecordsCount(); ++j, m_recordset2->MoveNext() )
		{
			mother_id	= m_recordset2->GetFieldString( MARRIAGES_SPOUSE2_ID );
			// anya adatai
