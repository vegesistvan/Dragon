...
void CEditTable::colorP()
{
	...
	m_type			= theApp.m_recordset->GetFieldString( TABLES_PERCENT );
	m_familyName	= theApp.m_recordset->GetFieldString( TABLES_FAMILY_NAME );
	m_alias			= theApp.m_recordset->GetFieldString( TABLES_ALIAS );
	m_titolo		= theApp.m_recordset->GetFieldString( TABLES_TITOLO );
	m_otherName 	= theApp.m_recordset->GetFieldString( TABLES_KNOWN_AS );
	m_roman			= theApp.m_recordset->GetFieldString( TABLES_TABLENUMBERROMAN );
	m_comment		= theApp.m_recordset->GetFieldString( TABLES_COMMENT );
	m_root			= theApp.m_recordset->GetFieldString( TABLES_TORZS );
	m_ag			= theApp.m_recordset->GetFieldString( TABLES_ARM );
	m_joint			= theApp.m_recordset->GetFieldString( TABLES_JOINT );    // 1: kapcsolódik, 0: nem kapcsolódik
	m_tableHeader	= theApp.m_recordset->GetFieldString( TABLES_TABLEHEADER );
	...
	GetDlgItem( IDC_EDIT_OTHERNAME )->SetWindowTextW( m_otherName );
...
void CEditTable::createTableHeader()
{
	...
	if( !m_otherName.IsEmpty() )
	{
		m_tableHeader2 += L" ";
		m_tableHeader2 += m_otherName;
	}
...
void CEditTable::OnBnClickedOk()
{
	...
	CString fields = L"familyNumber, tableHeader, percent, familyName, titolo, tableRoman, comment, torzs, known_as, alias, arm, joint";
	CString values;
	values.Format( L"'%d', '%s', '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'", m_familyNumber, tableHeader, m_type, m_familyName, m_titolo, m_roman, m_comment, m_root, m_otherName, m_alias, m_ag, m_joint ); 
	...
	{
		m_command.Format( L"UPDATE tables SET tableHeader='%s', percent='%s',familyName='%s',titolo='%s',tableRoman='%s',comment='%s', torzs='%s', known_as='%s', alias='%s',arm='%s',joint='%s' WHERE rowid='%s'",\
		tableHeader,\
		m_type,\
		m_familyName,\
		m_titolo,\
