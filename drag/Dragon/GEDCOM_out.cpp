...
BOOL CGedcomOut::OnInitDialog()
{
	...
	GetDlgItem(IDC_PATH)->SetWindowText(theApp.m_dbFolderPath);
...
//	if( _waccess( m_blobFolder, 0 ) ) _wmkdir( m_blobFolder);
	...
	// ged fájl készítése
	m_gedPathName.Format( L"%s\\%s", theApp.m_dbFolderPath, m_fileName );
...
//	vIndi.fams_id = nincs;
	...
	for( UINT i = 0; i < p_ListCtrl->GetItemCount(); ++i )
	{
		if( theApp.m_inputMode == MANUAL ) 
		{
			...
			if( p_ListCtrl->GetItemText( i, N_SEX ) == MAN )
				vIndi.sex = 'M';
			else
				vIndi.sex = 'F';
		}
		else if( theApp.m_inputMode == GEDCOM ) 
		{
			...
			if( p_ListCtrl->GetItemText( i, G_SEX ) == MAN )
				vIndi.sex = 'M';
			else
				vIndi.sex = 'F';
		}
		...
		else if( theApp.m_inputMode == GAHTML ) 
		{
			...
			if( p_ListCtrl->GetItemText( i, H_SEX ) == MAN )
				vIndi.sex = 'M';
			else
				vIndi.sex = 'F';
		}
...
void CGedcomOut::fillFAM()
{
	...
	{
		rowidH	= _wtoi(  theApp.m_recordset->GetFieldString(  DBM_HUSBAND_ID ) );
		rowidW	= _wtoi(  theApp.m_recordset->GetFieldString(  DBM_WIFE_ID ) );
		...
		vFam.place = theApp.m_recordset->GetFieldString(DBM_PLACE);
		vFam.date = theApp.m_recordset->GetFieldString(DBM_DATE);
...
void CGedcomOut::printHeader()
{
	...
	str = L"2 CORP Véges István\n" ;
	str = UnicodeToUtf8( str ); 
	fwprintf( fg, str );

	str = L"2 EMAIL veges.istvan@gmail.com\n";
	str = UnicodeToUtf8(str);
	fwprintf(fg, str);
...
void CGedcomOut::printINDI()
{
	...
	for( UINT i = 0; i < vINDI.size(); ++i )
	{
		...
		for( UINT j = 0; j < vPhotos.size(); ++j )
		{
			...
			{
				str.Format( L"1 FILE .\\%s\\%s\n", m_blobFolder, vPhotos.at(j).link );
				str = UnicodeToUtf8( str ); 
				fwprintf( fg, str );
			}
		}
